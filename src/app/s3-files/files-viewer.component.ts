...
@Component({
  ...
  <div style="text-align: center" *ngIf="!imagesShow; else seeImage">
    ...
    <mat-list style="width: 50%;margin: 0 auto;" role="list">
      <mat-list-item role="listitem" *ngFor="let item of fileList">{{item.shortAddress}}</mat-list-item>
    </mat-list>
  ...
  <ng-template #seeImage>
    <mat-grid-list cols="2" rowHeight="2:1.3">
      <mat-grid-tile *ngFor="let item of fileList">
        <mat-grid-tile-header>{{item.shortAddress}}</mat-grid-tile-header>
        <img mat-card-image style="width: 100%;" [src]="item.file" [alt]="item.shortAddress">
      </mat-grid-tile>
    </mat-grid-list>
  </ng-template>
    `,
...
export class FileViewerComponent implements OnInit {
  ...
  ngOnInit() {
    this.fileList.forEach(ele => {
      ele.shortAddress = ele.address.split("/").pop();
    });
  }
  ...
  async getImages() {
    ...
    this.fileList.forEach(ele => {
      ...
      this.http.post(environment.baseUrl+'files/'+this.funcNameUrl, body).subscribe(preSignedUrl => {
